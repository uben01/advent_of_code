...
func main() {
	...
	for i := 0; i < len(corruptedBlocks); i++ {
		fallBlocks(m, corruptedBlocks[0:i])

		success := bfs(m, coord{0, 0, 0})
		if !success {
			lastNonBlockerByte := corruptedBlocks[i-1]
			fmt.Printf("last non blocker byte: %d,%d", lastNonBlockerByte.x, lastNonBlockerByte.y)
			return
		}
	}
...
func bfs(m [][]rune, pos coord) bool {
	costs := make([][]int, size)
	for i := 0; i < size; i++ {
	...
	return costs[size-1][size-1] != math.MaxInt
}

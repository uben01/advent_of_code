...
fn find_longest_hike(map: &Vec<Vec<char>>, distances: &mut Vec<Vec<usize>>, start: &(usize, usize), finish: &(usize, usize)) -> usize {
    let rows = map.len();
    let columns = map[0].len();
    ...
    loop {
        ...
        if found_paths.len() > 1 {
            ...
            for found_path in found_paths {
                ...
                let result = find_longest_hike(map, &mut distances, &found_path, finish);
                if result > longest_hike {
                    longest_hike = result;
                }
            }
            return longest_hike;
